...
(function () {
  ...
  function addMenu() {
    ...
    let lockedScrollY=0;function toggle(force){const open=typeof force==='boolean'?force:!drawer.classList.contains('open');if(open){lockedScrollY=window.scrollY||window.pageYOffset||0;document.body.style.top=`-${lockedScrollY}px`;document.body.classList.add('library-menu-open')}else{document.body.classList.remove('library-menu-open');document.body.style.top='';window.scrollTo(0,lockedScrollY)}drawer.classList.toggle('open',open);scrim.classList.toggle('open',open);button.classList.toggle('open',open);button.setAttribute('aria-expanded',String(open));button.setAttribute('aria-label',open?'Close library menu':'Open library menu')}
